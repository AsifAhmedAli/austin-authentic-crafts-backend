...
const get_form_status = (req, res) => {
  try {
    // Retrieve all phone numbers from the database
    db.query("SELECT * FROM form_status", (err, results) => {
      if (err) {
        console.error("Error executing query:", err);
        return res.status(500).json({ error: "Internal Server Error" });
      }
      if (results.length !== 1) {
        return res.status(500).json({ error: "No Status in the database" });
      }
      // console.log();
      const status = results[0].form_status;
...
const change_form_status = (req, res) => {
  try {
    // Retrieve all phone numbers from the database
    db.query("SELECT * FROM form_status", (err, results) => {
      if (err) {
      ...
      const status = results[0].form_status;
      if(status == 1){
        db.query("Update form_status set form_status = 0", (err, results) => {
          if (err) {
            console.error("Error executing query:", err);
            return res.status(500).json({ error: "Internal Server Error" });
          }
            ...
            res.status(200).json({status: results[0].form_status});
          })
      }})
    }
      else{
        db.query("Update form_status set form_status = 1", (err, results) => {
          if (err) {
            console.error("Error executing query:", err);
            return res.status(500).json({ error: "Internal Server Error" });
          }
